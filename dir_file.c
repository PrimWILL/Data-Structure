...
void make_dir_and_file(directory_t* root_dir, char** token_list, int num_token) {
    find_create_file(directory_t* dir, char* name, bool* is_create)
    /*
    file_t *file;

	assert(dir != NULL);

	file = find_file(dir, name); // 해당되는 파일이 있다면 그 값을 리턴
                find_file(directory_t* dir, char* name) {
                    int i;
                    ...
                    return NULL; // 해당되는 파일이 없을 경우
                }
	
	if (file == NULL) {
		file = create_file(name); // 해당되는 파일 이름으로 리턴
		...
		*is_create = true; // 값이 생성되었습니다~
	}	// 자기 참조 구조체로 file명을 list에 저장된 값을 반환했으니 내가 root_dir에 저장할 것

	return file;
    */

   find_create_dir(directory_t* parent_dir, char* name, bool* is_create)
   /*
    directory_t *dir;
	...
	dir = find_dir(parent_dir, name);
                find_dir(directory_t* dir, char* name) {
                    int i;
	...
	return dir; // directory를 생성했으니 root_dir에 저장해라~
   */
}
